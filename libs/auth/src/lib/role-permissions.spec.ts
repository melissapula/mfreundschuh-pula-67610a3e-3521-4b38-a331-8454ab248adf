...
describe('role-permissions', () => {
    it('gives Viewer read-only access', () => {
        ...
        expect(roleHasPermission(Role.VIEWER, Permission.TASK_CREATE)).toBe(
            false,
        );
        expect(roleHasPermission(Role.VIEWER, Permission.TASK_UPDATE)).toBe(
            false,
        );
        expect(roleHasPermission(Role.VIEWER, Permission.TASK_DELETE)).toBe(
            false,
        );
        expect(roleHasPermission(Role.VIEWER, Permission.AUDIT_READ)).toBe(
            false,
        );
    });
